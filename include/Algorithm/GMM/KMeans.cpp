...
KMeans::KMeans(int dimNum, int clusterNum)
{
	...
	m_dissimilarityMode = EuclideanDistance;
...
void KMeans::Init(double *data, int sample_size)
{
	int size = sample_size;

	switch (m_initMode)
	{
	case KMeans::InitRandom:
	{
		int inteval = sample_size / m_clusterNum;
		...
		break;
	case KMeans::InitManual:
	{

	}
		break;
	case KMeans::InitUniform:
	{
		double* sample = new double[m_dimNum];

		for (int i = 0; i < m_clusterNum; i++)
		{
			int select = i * sample_size / m_clusterNum;
			for (int j = 0; j < m_dimNum; j++)
				sample[j] = data[select*m_dimNum + j];
			memcpy(m_means[i], sample, sizeof(double)* m_dimNum);
		}
...
double KMeans::CalcDistance(const double* x, const double* u, int dimNum)
{
	...
	switch (m_dissimilarityMode)
	{
	case KMeans::EuclideanDistance:
		...
		temp = sqrt(temp);
		break;
	case KMeans::ManhattanDistance:
		for (int d = 0; d < dimNum; d++)
		{
			temp += fabs(x[d] - u[d]);
		}
		break;
	case KMeans::MinkowskiDistance:		
		break;
	default:
		break;
	}
